...
TODO: this is not supported (Yet)
  ...
  struct dump_info di; 
  dump_begin (&di,TDI_class);
  if (!di.stream)
    return;  
  fprintf (di.stream, "Class %s\n", type_as_string (t, TFF_PLAIN_IDENTIFIER));
  fprintf (di.stream, "   size=%lu align=%lu\n",
	   (unsigned long)(tree_low_cst (TYPE_SIZE (t), 0) / BITS_PER_UNIT),
	   (unsigned long)(TYPE_ALIGN (t) / BITS_PER_UNIT));
  dump_class_hierarchy_r (di.stream, di.flags, t, TYPE_BINFO (t), 0);
  fprintf (di.stream, "\n");
  dump_end (&di,TDI_class);
*/
     ...
     tree vtable;
{
  ...
  if (!(di.flags & TDF_SLIM))
    {
      int ctor_vtbl_p = TYPE_BINFO (t) != binfo;
      
      fprintf (di.stream, "%s for %s",
	       ctor_vtbl_p ? "Construction vtable" : "Vtable",
	       type_as_string (binfo, TFF_PLAIN_IDENTIFIER));
      if (ctor_vtbl_p)
	{
	  if (!TREE_VIA_VIRTUAL (binfo))
	    fprintf (di.stream, " (0x%lx instance)", (unsigned long)binfo);
	  fprintf (di.stream, " in %s", type_as_string (t, TFF_PLAIN_IDENTIFIER));
	}
      fprintf (di.stream, "\n");
      dump_array (di.stream, vtable);
      fprintf (di.stream, "\n");
    }
     ...
     tree vtt;
{
  ...
  if (!(di.flags & TDF_SLIM))
    {
      fprintf (di.stream, "VTT for %s\n",
	       type_as_string (t, TFF_PLAIN_IDENTIFIER));
      dump_array (di.stream, vtt);
      fprintf (di.stream, "\n");
    }
  
  dump_end (di,TDI_class, stream);
*/
